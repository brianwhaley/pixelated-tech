...
describe('Blog page', () => {
	...
	it('renders the blog list when wordpress posts are available', async () => {
		...
		expect(screen.getByTestId('mock-blogpostlist').textContent).toContain('site:thethreemusesofbluffton.wpcomstaging.com count:1');
	});
	...
	it('renders the blog list when wordpress posts are unavailable', async () => {
		...
		render(<BlogPage />);
		await waitFor(() => expect(screen.getByTestId('mock-blogpostlist')).not.toBeNull());
		expect(screen.getByTestId('mock-blogpostlist').textContent).toContain('site:thethreemusesofbluffton.wpcomstaging.com count:0');
	});
});
