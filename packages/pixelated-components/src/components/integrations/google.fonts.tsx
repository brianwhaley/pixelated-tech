...
/**
 * Internal helper to encode font family names for Google Fonts URLs.
 * Handles Name:params and replaces spaces with +.
 */
function encodeFontFamily(family: string): string {
...
export function extractGoogleFonts(visualdesign: Record<string, any>): string[] {
	...
	for (const [key, token] of tokens) {
		...
		// 1. Explicit import_family takes priority
		...
		// 2. Fallback to *-font or *-primary keys if they are non-websafe
...
/**
 * Helper to build a single Google Font URL.
 */
...
export function generateGoogleFontsLink(fonts: string[]): string {
	...
	const links = families.map(family => {
		const url = getSingleFontUrl(family);
...
export interface GoogleFontsProps {
	/** Optional visualdesign object from siteconfig.json to automatically extract fonts from */
 ...
 */
export function GoogleFonts({ visualdesign }: GoogleFontsProps) {
