...
export function getUsStateName(abbr: string) {
	return US_STATES.find((state) => state.value === String(abbr).toUpperCase())?.text;
}

export function getUsStateAbbreviation(name: string) {
	return US_STATES.find((state) => state.text.toLowerCase() === String(name).trim().toLowerCase())?.value;
}
