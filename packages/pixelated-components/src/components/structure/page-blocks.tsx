 ...
 * @param param0 - Props containing the image URL.
 * @returns JSX.Element representing the page background.
 */
PageBg.propTypes = {
	image: PropTypes.string.isRequired,
};
export type PageBgType = InferProps<typeof PageBg.propTypes>;
export function PageBg(props: PageBgType) {
	const { image } = props;
	return (
...
// ========== PAGE HTML ==========
/**
 * PageHTML — Simple component to render raw HTML content.
 *
 * @param {string} [props.html] - Raw HTML string to render.
 * @param {node} [props.children] - Optional child nodes to render if `html` is not provided.
 * 
 * @returns A div containing the rendered HTML or children.
 */
PageHTML.propTypes = {
/** Raw HTML content */
	...
	/** Optional CSS class */
	className: PropTypes.string,
	/** Children to render as fallback or addition (node) */
	children: PropTypes.node
};
export type PageHTMLType = InferProps<typeof PageHTML.propTypes>;
export function PageHTML({ html, className, children }: PageHTMLType) {
	const combinedClassName = `page-html-content${className ? ` ${className}` : ""}`;
	if (html) {
		return <div className={combinedClassName} dangerouslySetInnerHTML={{ __html: html }} />;
	}
